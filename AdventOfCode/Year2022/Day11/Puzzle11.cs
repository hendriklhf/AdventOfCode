...
public sealed class Puzzle11 : Puzzle
{
    ...
    public int Solve()
    {
        ...
        for (int i = 1, m = 0; i < 55; i += 2)
        {
            ...
            Span<int> items = stackalloc int[30];
            byte itemCount = 0;
            while (true)
            {
        ...
        for (int round = 0; round < 20; round++)
        {
            for (int m = 0; m < 8; m++)
            {
                ...
                Span<int> items = monkey.Items;
                for (int i = 0; i < monkey.ItemCount; i++)
                {
                    int item = items[i];
                    int newValue = monkey.Inspect(item) / 3;
                    int target = monkey.GetThrowTarget(newValue);
        ...
        Span<int> topTwo = stackalloc int[]
        {
